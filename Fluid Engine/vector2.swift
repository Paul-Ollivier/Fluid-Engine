...
func monotonicCatmullRom(v0:SIMD2<Float>, v1:SIMD2<Float>,
    ...
    if (abs(D1.x) < Float.leastNonzeroMagnitude ||
        sign(1, D1.x) != sign(1, d1.x) ||
        sign(1, D1.x) != sign(1, d2.x)) {
        d1.x = 0
        d2.x = 0
    }
    
    if (abs(D1.y) < Float.leastNonzeroMagnitude ||
        sign(1, D1.y) != sign(1, d1.y) ||
        sign(1, D1.y) != sign(1, d2.y)) {
        d1.y = 0
        d2.y = 0
    }
...
func monotonicCatmullRom(v0:SIMD2<Double>, v1:SIMD2<Double>,
    ...
    if (abs(D1.x) < Double.leastNonzeroMagnitude ||
        sign(1, Float(D1.x)) != sign(1, Float(d1.x)) ||
        sign(1, Float(D1.x)) != sign(1, Float(d2.x))) {
        d1.x = 0
        d2.x = 0
    }
    
    if (abs(D1.y) < Double.leastNonzeroMagnitude ||
        sign(1, Float(D1.y)) != sign(1, Float(d1.y)) ||
        sign(1, Float(D1.y)) != sign(1, Float(d2.y))) {
        d1.y = 0
        d2.y = 0
    }
